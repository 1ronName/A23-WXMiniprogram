...
Page({
  ...
  async loadDraftState(options) {
    ...
    if (sourceDocIds.length <= 0 || !draft.templateLocalPath) {
  ...
  async refreshSelectedSources(options) {
    ...
    try {
      ...
      this.setData({
        flowReady: runDocs.length > 0 && Boolean(draft.templateLocalPath),
        sourceDocs: runDocs,
        selectionSummary,
        templateName: draft.templateName || '',
        templateLocalPath: draft.templateLocalPath || '',
  ...
  removeSourceDoc(e) {
    ...
    this.setData({
      sourceDocs: nextDocs,
      selectionSummary,
      flowReady: nextDocs.length > 0 && Boolean(this.data.templateLocalPath),
      executionBlockedText: this.buildExecutionBlockedText(selectionSummary),
    })
  },
  ...
  async startAutofill() {
    ...
    try {
      ...
      const templateId = String((templateInfo && (templateInfo.id || templateInfo.templateId)) || '')
      if (!templateId) {
